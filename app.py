    ...
    if disease.lower() == "yes":
        disease_expert = Agent(
            ...
            backstory=f"""Specialized in dealing with individuals having {disease}. 
                          ...
                          Do not prescribe medicines but only give advice. Answer in Spanish""",
            verbose=True,
            llm=llm,
            allow_delegation=True,
        )
        ...
        task1 = Task(
            description=f"""Analyze the fitness requirements for a {age}-year-old {gender}. 
                            Provide recommendations for exercise routines and fitness strategies. Answer in Spanish""",
        ...
        task2 = Task(
            description=f"""Assess nutritional requirements for a {age}-year-old {gender}. 
                        ...
                        Do not prescribe a medicine. Answer in Spanish""",
            agent=nutritionist,
            llm=llm
        )
        ...
        task3 = Task(
            description=f"""Evaluate overall health considerations for a {age}-year-old {gender}. 
                        Provide recommendations for a healthy lifestyle. Answer in Spanish""",
            agent=doctor,
            llm=llm
        )
